...
export function ProjectCard({
  ...
  const titleNode = href ? (
    <Link
      href={href}
      className="group-hover:decoration-border-strong text-[19px] leading-tight font-medium tracking-[-0.02em] underline decoration-transparent underline-offset-[6px] transition-[text-decoration-color] duration-200 ease-out"
    >
      {title}
    </Link>
  ) : (
  ...
  return (
    ...
    <li className="group border-border grid grid-cols-[2rem_1fr] gap-x-1 border-b pb-6 last:border-b-0 last:pb-0">
      <span className="text-subtle pt-1 font-mono text-[12px] tracking-[0.05em] tabular-nums">
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
          ...
          {meta && (
            <span className="text-subtle flex shrink-0 items-center gap-2 font-mono text-[12px] tracking-[0.05em]">
              {status === "wip" && (
                <span className="bg-accent size-[5px] animate-pulse rounded-full" />
              )}
              {meta}
            </span>
          )}
        ...
        {tagline && (
          <p className="text-foreground mt-1 flex gap-1.5 text-[15px]">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2">
            {slug && (
              <Link
                ...
                className="link-retract text-accent text-[15px] transition-opacity duration-150 ease-out active:opacity-70"
              >
                Read more
              </Link>
            )}
